...
const MetricCard: React.FC<{ icon: React.ReactNode; label: string; value: string | number; color: string }> = ({ icon, label, value, color }) => (
  <div className={`flex-1 p-4 bg-gray-900/50 rounded-lg border border-gray-700 flex flex-col items-center justify-center text-center min-w-[140px]`}>
    <div className={`flex items-center justify-center w-12 h-12 rounded-full mb-3 bg-${color}-500/10`}>
      {React.cloneElement(icon as React.ReactElement, { className: `w-6 h-6 text-${color}-400` })}
    </div>
    <p className="text-sm text-gray-400">{label}</p>
    <p className={`text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-${color}-400 to-${color}-200`}>{value}</p>
  </div>
);
